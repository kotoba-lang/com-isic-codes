(ns isic_codes.main
  "Kotodama WASM entrypoint for the isic_codes clean-room actor (L4) — Clojure port.
  ...
  facts (`isic_codes.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "isic_codes")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"isic_codes.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "isic_codes-compat" :tier tier :entities entities} 200])
...
(defn start! [] :isic_codes-compat/ready)
